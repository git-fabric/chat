...
// ── Public API ────────────────────────────────────────────────────────────────
export async function listTools(gatewayUrl) {
    const raw = (await mcpRequest(gatewayUrl, "tools/list"));
...
export async function callTool(gatewayUrl, name, args) {
    const raw = (await mcpRequest(gatewayUrl, "tools/call", { name, arguments: args }));
...
export async function selectRelevantTools(allTools, userMessage, anthropic) {
    ...
    try {
        const response = await anthropic.messages.create({
            ...
            max_tokens: 128,
            system: "You are a tool router. Given a user message and a list of tool categories, return ONLY the category prefixes that are relevant. Reply with a JSON array of strings, nothing else. Example: [\"k8s\",\"pve\"]",
            messages: [
                {
                    role: "user",
        ...
        if (!jsonMatch)
            return allTools;
        const selectedPrefixes = JSON.parse(jsonMatch[0]);
        if (!Array.isArray(selectedPrefixes) || selectedPrefixes.length === 0)
            return allTools;
